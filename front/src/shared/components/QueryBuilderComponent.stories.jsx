...
|-------------------|------------------------------------|-------------|
| \`fields\`         | \`{ name, label, type }[]\`         | Lista de campos disponibles para reglas. |
| \`defaultQuery\`   | \`object\`                          | Estado inicial del query. Por defecto: combinator "and" y sin reglas. |
| \`title\`          | \`string\`                          | Título que aparece en la parte superior del constructor. |
| \`onApply(query)\` | \`function\`                        | Se dispara al hacer clic en "Ejecutar". |
| \`onClear()\`      | \`function\`                        | Se dispara al hacer clic en "Limpiar". |
| \`onChange(query)\`| \`function\`                        | Se dispara en cada modificación del query. |
| \`onSaveTemplate(query)\` | \`function\`                | Callback al guardar una plantilla personalizada. |
| \`onDeleteTemplate(query)\` | \`function\`              | Callback al eliminar una plantilla personalizada. |

---

### 📌 Notas

- El estado del query es manejado internamente con \`useState\`.
- Puedes adaptarlo fácilmente para control externo si lo necesitas.
- 100% compatible con dispositivos móviles.
- Los estilos son 100% en TailwindCSS, sin dependencias de UI externas.
        `,
      },
    },
  },
  argTypes: {
    fields: {
      control: 'object',
      description: 'Lista de campos disponibles para construir las reglas.',
    },
    defaultQuery: {
      control: 'object',
      description: 'Query inicial (combinator y reglas definidas).',
    },
    title: {
      control: 'text',
      description: 'Título que se muestra en el encabezado del constructor.',
    },
    onApply: {
      action: 'onApply',
      description: 'Callback al presionar el botón "Ejecutar".',
    },
    onClear: {
      action: 'onClear',
      description: 'Callback al presionar el botón "Limpiar".',
    },
    onChange: {
      action: 'onChange',
      description: 'Callback que se dispara cada vez que el query cambia.',
    },
    onSaveTemplate: {
      action: 'onSaveTemplate',
      description: 'Callback que se dispara al guardar una plantilla.',
    },
    onDeleteTemplate: {
      action: 'onDeleteTemplate',
      description: 'Callback que se dispara al eliminar una plantilla.',
    },
  },
};

// 🔹 Template base
const Template = (args) => <QueryBuilderComponent {...args} />;
...
export const ConCamposYQuery = Template.bind({});
ConCamposYQuery.args = {
  fields: [
    { name: 'nombre', label: 'Nombre', type: 'string' },
    { name: 'edad', label: 'Edad', type: 'number' },
    { name: 'activo', label: 'Activo', type: 'boolean' },
    { name: 'ciudad', label: 'Ciudad', type: 'string' },
    { name: 'fecha', label: 'Fecha de alta', type: 'date' },
  ],
  defaultQuery: {
    combinator: 'and',
    rules: [
...
// 🔹 Story con callbacks funcionales
export const ConCallbacks = () => {
  const fields = [
    { name: 'nombre', label: 'Nombre', type: 'string' },
    { name: 'edad', label: 'Edad', type: 'number' },
    {
  ...
  const onSaveTemplate = (query) => {
    alert('Plantilla guardada:\n' + JSON.stringify(query, null, 2));
  };

  const onDeleteTemplate = (query) => {
    alert('Plantilla eliminada:\n' + JSON.stringify(query, null, 2));
  };

  return (
    <QueryBuilderComponent
      title="QueryBuilder con callbacks"
      fields={fields}
      onApply={handleApply}
      onClear={handleClear}
